...
- `inspect`: bounded run, registry, ORCH, and budget observation. The budget view neither judges nor unparks anything, but the call runs the settlement sweep, so it may persist settlement facts — and, riding that write, materialize a legacy registry at the current schema — and then reports `effect: "confirmed"` with the new revision. It runs the settlement sweep first, like `wait` and `herdr_worker inspect`, so whoever looks learns the truth: a lane that reported completion settles here, and a report whose block does not parse says so in `data.settlement_sweep`. The revision it returns is the post-sweep one, so a `close` can be issued from it directly.
