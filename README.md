...
A grant is a decision, not automatically a ceiling. `budget_extend` and `inspect` report the same per-axis block: the audit `verdict`, `granted`, `effective_cap`, `usage`, `applied`, and `usable`. `applied` says whether the granted figure is in force on that axis and, when it is not, why — `awaiting-clamp` (a human has to raise the clamp; every `full`-policy grant starts here), `pinned` (a human ceiling sits below the figure), `write-owed` (the server's own clamp write has not landed and is retried), or `none` (nothing was granted on that axis). `usable` is a different question — usage against the axis ceiling right now — so a grant can be `awaiting-clamp` on an axis that is still usable because your clamp was already high enough. When a park is waiting on you, the response names `required_clamp`: the exact field and value to write into `budget-clamp.json`, plus one `release_condition` sentence. Writing it does not release the run by itself; the park lifts at the next guarded op that judges the budget, and `budget_extend` re-judges before its own gates so the call you make right after editing the file is enough.
