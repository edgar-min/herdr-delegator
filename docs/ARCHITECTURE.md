...
`plan.md`, `evidence.md`, assignment files, and reports exist only when authored. Initialization does not create placeholders. A run created under the packaged-delivery backing records renders no `guidance.md` and no `role-skills/` at all — its role instructions are the installed packaged skills and its advisory criteria ride inline in the prompt. `guidance.md` is still rendered by `open` and by both `revive` modes for a run whose backing bytes predate packaged delivery, never by `init`, and every layout and reconcile check tolerates its absence and its presence, so runs of every era still load.

Which delivery a run gets is decided at one required-input boundary shared by the ORCH spawn, direct dispatch, FIFO promotion and the guidance-materialization gate: the run's own `protocol-orch.md` / `protocol-worker.md` is read as a canonical regular file and passed through the same shipped-digest acceptance `init` uses, and only those accepted bytes are then classified as packaged, marked-historical or unmarked-historical. A missing, unreadable or unaccepted record is a refusal — never a silent fall-through to a historical branch — and the refusal happens before the assignment is marked `prompting`, before `prompted_at`, and before anything is submitted to the adapter, so no fallback artifact is written and no lane is told about work it cannot be given instructions for.
...
A sixth section, `References`, MAY follow as the last one. It pins documents by hash — `- <run-relative path> sha256:<64 hex>`, at most 16 — and each path must resolve to a plain file inside the run directory (no `..`, symlink, hardlink, duplicate `(dev, ino)`, or file over 256 KiB). `preflight` and `add` verify every hash and refuse before the ID is consumed; from the first dispatch onward a mismatch is a `reference_drift` observation on `wait`, `herdr_worker inspect`, and `herdr_track close`, never a recall.

The artifact becomes immutable when its SHA-256 is submitted as `instructions_sha256`, and `add` makes the file mode `0444` so the filesystem says the same thing. A failed `chmod` is reported as `assignment_readonly_failed` and never unwinds the registration. No contract JSON or receipt JSON exists.
...
1. verify the assignment file, grammar, coordinates, hash, pinned references, and the lane profile a live lane was launched under;
...
Resolved configuration reaches a reader as a pure projection — the renderer authors no sentences; every content block maps to a config coordinate, plus a closed set of fixed structural strings. On a packaged run the projection is delivered inline with the prompt: the orchestrator's first prompt carries its `directive` first, then the worker-profile selection table (each profile's `intent`; the legacy `guidance` field serves as `intent` fallback) and the orch-moment routes with each skill's `intent` and `trigger`; a lane's assignment prompt carries only that profile's `directive` and its intake/report routes. Historical runs keep the two rendered documents: `guidance.md` at `open` and both revival modes, `guidance-<profile>.md` at dispatch, named in the dispatch pointer as advisory, and the marked role-template era embeds the same blocks beneath the generated role body. Absence is a no-op everywhere: a profile with neither directive nor routes yields no advisory block, no document and no pointer; an advisory failure degrades to a named-failure document or a warning and never blocks a birth or dispatch, while a required packaged role skill that cannot be resolved fails the call before any prompt effect.
...
An extension costs a bounded justification, obeys a per-axis step cap and a minimum interval, and grants nothing until a clean auditor — spawned by the server, never by the ORCH, and not a responsibility lane — appends a verdict the server records. Either axis may be asked for, `requested_tokens` or `requested_minutes` or both, an over-ambitious ask is truncated to the step rather than refused, and an omitted axis still moves by its own step because a verdict disposes of both dimensions. The auditor's answer is per axis too: a `grant` approves the whole request and a `deny` approves nothing, while a `partial` cuts each axis it names — `granted_tokens`, and optionally `granted_minutes` — and approves in full any axis it leaves out. Without that second lever a `partial` could only cut spend, so a run whose real problem was wall clock got its tokens trimmed and its minutes granted in full. A deny ends the ladder at the user. Fresh open scaffolds `budget-clamp.json`; every park path scaffolds it lazily for older runs. Creation is owner-only and never overwrites an existing file. Park/deny recovery and ledger entries name its exact editable schema `{version:1, max_tokens?, max_minutes?, note?}`. Money units and precise cost accounting remain out of scope.

A verdict is a decision, not its application, and conflating the two was a real defect: a `full`-policy run read verdict `grant` with `granted_tokens` 600,000 while its effective cap stayed 400,000 and the run stayed parked on `approval-required`, and no field anywhere said which of the two figures was in force. Every settled extension now carries a per-axis `applied` state — `applied`, `awaiting-clamp`, `pinned`, `write-owed`, `none` — whose discriminator is observable at settlement: did the axis ceiling carry the granted figure, and if not, what held it down. `usable` is a separate observation of the moment, axis usage against the axis cap, so a grant can read `awaiting-clamp` on an axis that is still usable because the human's ceiling was already high enough. `budget_extend` and `inspect` assemble that report from one implementation, so the two read the same semantics from the same inputs — a later observation may still differ, because metering and the clamp move on, and every refusal and observation carries `required_clamp`: the exact field and value a human writes to release the park, and nothing at all when no axis is held below an approved figure, because then the route is another extension. Applying an approval and releasing a park are different acts: where spend has already passed the approved figure, writing it applies the approval and the park stands, so each entry also carries `releases_at` — the lowest ceiling that would clear the current judgment, reported as arithmetic rather than as the server choosing a human's ceiling — and `release_condition` names the further extension or the higher ceiling as the remaining route. A `denied` park has no verdict route at all; only a human clamp change reopens it. Release is a conjunction over both axes at one judgment. Nothing an observation reports can itself unpark a run — the transition belongs to the next guarded op that judges the budget, and `budget_extend` makes that judgment before its own pinned/denied/interval gates so a ceiling just written by hand releases the very call that reads it. "Read-only" is said of the budget state alone: an observation never judges, unparks, moves a cap or rewrites an applied state, while the settlement sweep that `herdr_track inspect` and `close` run is a registry write older than this design — so an observation may persist settlement facts and, on that same write, materialize a legacy registry at the current schema, reporting `effect: "confirmed"` and the new revision rather than posing as a pure read. An `applied` value is history too: it records how a grant landed at its settlement, so a later clamp change leaves it standing, and current permission is read from `effective_cap`, `usable` and `park_reason`.

The two axes are also held symmetrically. Under `full` a verdict alone raises neither: tokens are held at `seed_tokens` and wall clock at `minutes_floor`, and the human applies an approved figure by writing `max_tokens` or `max_minutes`. The token axis worked that way from the start while the minutes cap simply followed `granted_minutes`, so a `full` run could be parked for approval on tokens while its minutes rose with no approval at all. Migration preserves rather than retracts: a registry written before `minutes_floor` existed is read with `max(seed_minutes, granted_minutes)` projected in memory and materializes that value once, inside the first guarded mutation's transaction. A budget read never writes — `inspect` neither judges nor unparks, though the settlement sweep in the same call may write, and the call then reports `effect: "confirmed"` with the new revision when it did and `effect: "none"` when it only observed — and preserving a ceiling the previous rule already allowed is not a new approval — the server still has no path by which it writes `max_minutes`.
