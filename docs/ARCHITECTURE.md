...
A verdict is a decision, not its application, and conflating the two was a real defect: a `full`-policy run read verdict `grant` with `granted_tokens` 600,000 while its effective cap stayed 400,000 and the run stayed parked on `approval-required`, and no field anywhere said which of the two figures was in force. Every settled extension now carries a per-axis `applied` state — `applied`, `awaiting-clamp`, `pinned`, `write-owed`, `none` — whose discriminator is observable: does the axis ceiling carry the granted figure, and if not, what holds it down. `usable` is a separate observation of the moment, axis usage against the axis cap, so a grant can read `awaiting-clamp` on an axis that is still usable because the human's ceiling was already high enough. `budget_extend` and `inspect` assemble that report from one implementation, so the response and the inspect that follows it cannot disagree, and every refusal and observation carries `required_clamp`: the exact field and value a human writes to release the park, and nothing at all when no axis is held below an approved figure, because then the route is another extension. Nothing an observation reports can itself unpark a run — the transition belongs to the next guarded op that judges the budget, and `budget_extend` makes that judgment before its own pinned/denied/interval gates so a ceiling just written by hand releases the very call that reads it.
