...
This document is the normative architecture and Markdown review artifact for `herdr-delegator` 2.0.0 and the bundled `herdr-delegation` skill 2.0.0.
...
- **ID-002**: The package and plugin version MUST be `2.0.0`.
- **ID-003**: The public skill MUST be named `herdr-delegation` and versioned `2.0.0` under frontmatter metadata.
...
- **RUN-001d**: `open` MUST pin the creator session's entire observed role table from bridge facts into the registry as optional `pinned_roles` — each role mapped to its concrete provider/model and, when observed, its bound thinking — with provenance (observed session id, observation time, source), in the same registry mutation that stamps the creator record.
...
- **MOD-007**: When the run registry carries `pinned_roles`, spawn resolution (orchestrator profile, worker launch profiles, role-bound thinking, and the silent-fallback warning judgment) MUST resolve a configured role from the pinned table in preference to the live session's role resolutions. A registry without the table, or a role absent from it, MUST resolve live with a surfaced per-role warning — never fail closed. The pinned table is an input to resolution only and MUST NOT relax MOD-001/MOD-002 verification: spawns still pin explicit concrete provider/model/thinking and bootstrap/JSONL exact-match gates are unchanged. Tradeoff: OMP model-configuration changes made mid-run are deliberately not reflected in that run's resolutions; a new run observes them.
...
- **PKG-001**: Root `plugin.json` MUST conform to Agent Plugins 1.0.0, identify `herdr-delegator` version 2.0.0, and contain client-specific OMP data only under `extensions.io.github.edgar-min.herdr-delegator`.
...
- **PKG-003**: Agent Plugins portable authority MUST remain `plugin.json`, `skills/`, and `mcp.json`. `package.json` MUST remain npm/current-OMP compatibility metadata with version 2.0.0, direct runtime dependencies, and only the namespaced `omp.extensions` entry.
...
- [ ] Agent Plugins `plugin.json`, Agent Skills frontmatter, package metadata, and skill metadata identify version 2.0.0.
