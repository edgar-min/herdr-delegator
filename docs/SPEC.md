...
This document is the normative architecture and Markdown review artifact for `herdr-delegator` 3.7.0 and the bundled `herdr-delegation` skill 3.7.0.
...
- **ID-002**: The package and plugin version MUST be `3.7.0`.
- **ID-003**: The public skill MUST be named `herdr-delegation` and versioned `3.7.0` under frontmatter metadata.
...
- **RSP-003**: A lane MUST have at most one active assignment and one queue whose order is fixed by its insertion discipline: an `add` MUST append by default, and MUST insert at the head when the call asks for it. Order MUST be readable from the queue itself; no rank, priority, or class may be persisted on an assignment record to reconstruct it.
...
- **ASN-003b**: The artifact grammar is not schema-versioned, so the fourth field is a one-way relaxation: a label-free artifact parses under every shipped server, but a server built before ASN-003 rejects a label-bearing artifact as `assignment_artifact_invalid` with recovery text that tells the author to strip a healthy field. Before the first label-bearing artifact is authored on a shared working tree, every long-lived mounted server that may read that run MUST be respawned (`/reload-plugins`); this is an operator contract — nothing enforces it mechanically (friction 9facc09bdd08e951).
...
- **ASN-013**: Settlement MUST transition the assignment to its terminal state, return the lane to `idle`, record the last completed assignment, and promote the queue head.
...
- **BUD-004**: The budget MUST be judged at every guarded op. While parked, only a landing allowlist MUST run: assignment `wait`, worker `close`, track `close`, `budget_extend`, doorbells, and read-only actions. A queued head MUST NOT be dispatched — including on the emergency carve-out of BUD-016, which admits one registration and never a promotion — and `add` and worker `resume` MUST fail `budget_parked` except for an admissible emergency `add`.
...
- **BUD-009**: The server — never the ORCH — MUST spawn the auditor as a clean session on the `slow` worker profile, seed its document with the request and machine facts, and record the verdict server-side. The auditor MUST NOT be a responsibility lane, MUST NOT be addressable by the ORCH, and MUST be closed once settled, with unclosed auditors swept at later budget ops. After appending its verdict block the auditor MAY ring the ORCH pane exactly once through `wake_orch_audit`; that bell MUST carry no verdict content, MUST NOT be the surface any verdict is read from, and MUST NOT make the auditor addressable in return. Both audit document renders and the spawn prompt MUST state that the document alone is the authority. The "exactly once" and "auditor only" halves are prose obligations on the session, not gates: no per-session tool gating exists in this design, so the action MUST refuse only the run's own ORCH and any registered lane, and its safety MUST rest on the bell carrying nothing — a bell rung early, twice, or by another caller can cost the run nothing but a wasted read, because every verdict is read from its document (NG-014's stance on unprovable authorship, applied to a doorbell).
...
- **BUD-015**: The seed's two axes MUST be published as independent ceilings whose narrower side parks the run, so the judgment a seed really declares is its implicit rate — tokens divided by minutes. The binding surface for that criterion is the `mandate.budget` schema `describe` text (JDG-001), which MUST direct the caller to compare the declared rate against runs the mandate resembles rather than supply a formula; the measured spread across this project's own closed runs and its diagnostic reading belong in `docs/ARCHITECTURE.md`, not in a describe string (JDG-002). A successful `close` MUST append one `metered:` line to the append-only budget ledger, so a run that finished inside its seed leaves the same cost record as one that parked.
- **BUD-016**: A run parked for the cadence reason `over-cap` MUST admit one `add` that carries an `emergency` claim of `failure` and `why_now`. Each MUST be normalized and bounded to one line of at most the published justification limit before it is judged, because both are rendered verbatim into a document a clean auditor reads. The carve-out MUST buy registration only: no cap MUST move, the park MUST stand, and a queued head MUST NOT be dispatched; the registration it admits MAY still be dispatched, which is the whole point of admitting it. The carve-out authority MUST be refused as `emergency_not_admissible` under every other park reason — `denied`, `approval-required`, and `clamp-unreadable` are the human's decision and `audit-unavailable` is a broken machine — while an unparked run, which needs no carve-out, MUST accept the add and record the claim as unused rather than refuse it. Admission MUST create `emergency-audit-<n>.md` with create-exclusive semantics, which is also the concurrency arbiter, and MUST persist the obligation in that document alone: the file existing without a verdict block IS the open debt, so no registry key and no schema version MUST change, and a concurrent loser MUST be refused. At most one unjudged emergency registration MUST exist per run. The next `budget_extend` MUST run that post-hoc audit as its leading step, and that step MUST NOT be able to grant or fail the extension it precedes; a verdict it observes MUST reach the append-only ledger exactly once, whether this call's auditor wrote it or an earlier one did. A verdict MUST be `justified` or `unjustified` with no partial form, MUST close the debt either way, and MUST be recalled by nothing: no tool op cancels a dispatched assignment, closes a working lane, or un-spends tokens. The sanction MUST therefore be prospective only, and it MUST be enforced rather than narrated: `unjustified` closes the carve-out for the run permanently AND ends the escalation ladder at the human, so every later `budget_extend` MUST be refused `emergency_carve_out_closed` with the human-owned clamp named as the release. Every surface that offers the carve-out MUST state that recall is impossible.
...
- **OBS-003**: A terminal assignment MAY persist `advisory_unowned_changes` with exact fields `advisory: true`, `paths`, and `truncated`; `paths` MUST be sorted project-relative paths outside the union of declared ownership for active or settling assignments, limited to 64 entries of 1–1,024 UTF-8 bytes each, and `truncated` MUST report exclusion by the entry-count or entry-length bound. The union MUST be built only from declarations a strict lexical classifier accepts — a bare or backticked relative Git path, or such a path with a terminal `/**` read as its prefix — each resolved and lexically contained under the run's project directory; every other bullet MUST count as unclassified rather than be treated as a path, and both accepted classes MUST keep prefix coverage of what lives under them.
- **OBS-004**: The unowned-change audit MUST run as a best-effort, fail-open `git status --porcelain=v1 -z --untracked-files=normal` observation with optional locks and terminal prompts disabled, a 2,000 ms abort boundary, a 128 KiB output acceptance bound, and both source and destination paths checked for renames and copies. Any non-git directory, command error, timeout, malformed or ambiguous output, unsafe path, or ownership-artifact error MUST omit the observation without blocking settlement or changing terminal state, and the audit MUST NOT attribute a path to any worker. Every such skip MUST name itself as a bounded `<code>: <detail>` warning drawn from `git_audit_command_failed`, `git_audit_output_oversized`, `git_audit_output_non_utf8`, `git_audit_output_unterminated`, `git_audit_porcelain_invalid`, `git_audit_rename_invalid`, `git_audit_path_invalid`, `git_audit_path_outside_cwd`, `git_audit_timeout`, and `git_audit_exception`; unclassified or unreadable ownership MUST name itself as `ownership_declarations_unclassified` or `ownership_artifacts_unreadable`, and a partly classified input MUST still audit its classified subset. These warnings MUST appear only in the response of the call that made the observation and MUST NOT be persisted, and the audit MUST NOT prevent, refuse, or guard any edit, staging, or commit.
...
- **LIFE-008**: A reset copies planning context, not truth; inherited evidence MUST be revalidated. The enforcement point for that revalidation is the succession claim gate in §8.3, which refuses a succession run's first `herdr_assignment add` on claim presentation and sha freshness; every other aspect of revalidation stays the successor ORCH's judgment.
...
### 8.2 Inbound channel observation

- **INB-001**: A doorbell carries no content, so a bell the target could not receive MUST NOT be queued, replayed, or redelivered; what a run MUST be able to recover instead is the knowledge that a durable channel document is addressed to it. Inbound discovery is therefore an observation of documents on disk, and delivery guarantees remain a named non-goal.
- **INB-002**: The observation MUST be derived entirely at read time. It MUST NOT create persistent state, a registry field, a schema version event, a `delivery` value, or any write; a run whose registry predates the current schema version MUST remain observable, because the observation reads only the storage-root index, the run manifest, and channel-document bytes.
- **INB-003**: The observation MUST be content-free. Each entry MUST carry only the sender's `from_track_id` and `from_run_id`, the document `path`, its `sha256`, `bytes`, and `lines`, and the optional `entry_line` anchor of the newest entry — never a line of the document body.
- **INB-004**: Enumeration MUST be the storage-root index rather than a filesystem walk, MUST exclude the observing run's own coordinate, and MUST observe exactly one deterministic path per candidate: the sender-owned `a2a/orch-to-<target_track_id>_<target_run_id>.md`. Candidates MUST be ordered by descending `created_at` with `<track_id>/<run_id>` ascending as the tie-break, and that order MUST be applied before any limit, so a bounded scan is reproducible.
- **INB-005**: The scan MUST be bounded: at most 256 sender candidates and at most 32 entries by default, and a candidate document above 1 MiB or not a regular non-symlink file MUST be skipped. Exceeding a limit MUST set `truncated` and MUST name the limit in `observation_warning`.
- **INB-006**: Degradation MUST be loud and MUST NOT throw at a spawn or read boundary. An unreadable index or run manifest MUST return no entries with a non-empty `observation_warning`; a single unreadable candidate MUST leave the remaining entries intact and add a warning. A silently empty observation MUST NOT be returned for a failed scan, and an absent document MUST NOT be reported as a failure.
- **INB-007**: The observation MUST surface at exactly two read surfaces. `herdr_track inspect` MUST return it as `data.inbound_channels`, and the ORCH first prompt MUST name at most 8 observed documents, directing the reader to `herdr_track inspect` for the remainder whenever more exist or the scan was truncated. The prompt MUST name paths only, MUST claim only that a future or live ORCH can discover them, and MUST NOT claim delivery, completeness, or redelivery. An observation failure MUST NOT fail the spawn.
- **INB-008**: A non-delivered `notify_run` — `target_unresolved`, `rejected_blocked`, or `failed` — MUST additionally warn that the channel document remains durable at its path with its SHA-256 and byte count, that a future or live target ORCH can discover it from its first prompt or `herdr_track inspect`, that the tool does not guarantee redelivery, and that escalation is the caller's decision. The original failure reason MUST be preserved, and the `wake_orch`, `wake_peer`, and `wake_worker` warnings MUST be unchanged.

### 8.3 Succession claims

- **SUC-001**: The canonical succession coordinate MUST be `<run>/handoff.md`, and the gate MUST fire only when that path is an existing canonical regular file that carries exactly one heading of depth 2 through 4 titled `Inherited claims`. An absent file, a non-canonical path, or a document without that section MUST leave `herdr_assignment add` behavior, refusals, and result shape completely unchanged; more than one such heading MUST be refused as unparsable. An author-chosen filename MUST NOT be promoted to the canonical coordinate, because a document the machine cannot find is what made every previous revalidation clause unenforceable.
- **SUC-002**: The claims section MUST carry exactly one Markdown table whose header row is exactly `| claim | coordinate | command | observed | disposition |` followed by a five-cell delimiter row. Each subsequent row MUST parse as exactly five cells; `\|` MUST be read as a literal pipe, a backtick-wrapped cell MUST be unwrapped, and a cell holding only `-` MUST be read as empty. A `coordinate` MUST be a repo-relative or run-relative path with an optional `:<line>` suffix and MUST NOT be absolute or contain a `..` segment. The document MUST be valid UTF-8 within 256 KiB, each cell within 1,024 bytes, and the table within 256 rows; a zero-row table MUST be accepted as the honest declaration that no claim is inherited.
- **SUC-003**: `disposition` MUST be closed at exactly `measured`, `unverified`, `withdrawn`, and any other value MUST be a parse failure rather than a third state. Every row MUST state a non-empty `claim`. A `measured` row MUST carry `coordinate`, `command`, and `observed` as `<40-hex sha> <ISO-8601 instant with an explicit zone>`. An `unverified` row MUST leave `command` empty, so a claim nobody ran cannot be presented as one that was. A `withdrawn` row MUST require only the claim.
- **SUC-004**: Every `measured` row's observed sha MUST equal the current HEAD of the run's project directory, observed read-only with `git rev-parse HEAD` under a 2,000 ms abort boundary and a 4,096-byte output bound. A mismatch MUST refuse `succession_claim_stale`. A directory outside any Git work tree, a command error, a timeout, or output that is not a 40-hex object name MUST NOT block the operation: the syntactic checks of SUC-002 and SUC-003 MUST still refuse, and only the freshness comparison MUST be skipped under a bounded `succession_head_command_failed`, `succession_head_timeout`, `succession_head_output_invalid`, or `succession_head_exception` warning carried in the result. This fail-open disposition is the same family as OBS-004.
- **SUC-005**: A non-withdrawn claim that carries compatibility or version vocabulary, or that names a direction token at all, MUST state exactly one direction token — `old->new` or `new->old`, with `→` read as `->` — and the table MUST carry both directions. A one-directional compatibility claim MUST be refused as unparsable with the missing direction named, because a grammar measured only in the reading direction that was changed is how a document written for a newer reader became unreadable to the older one. A withdrawn claim MUST NOT be asked for its counter-direction.
- **SUC-006**: The gate MUST NOT execute anything in the `command` column; the column exists so a human or a successor can re-run the measurement, and a tool that ran it would be an arbitrary execution surface. A refusal MUST be a `succession_claims_unparsable` or `succession_claim_stale` `McpContractError` in phase `validate`, non-retryable, non-ambiguous, mutating no registry state, and its recovery MUST name each failing row's claim with the format or sha it needed. A pass MUST be exposed only as the non-persisted observation `data.succession` — row count, per-disposition counts, and the observed HEAD — and MUST NOT be recorded anywhere, because a stored "the gate passed" is the same replayed-proof category SRT-004 and COM-004 forbid. The gate MUST NOT be represented as verifying that any claim is true; presentation and freshness are decidable, and truth is not.
...
- **PKG-001**: Root `plugin.json` MUST conform to Agent Plugins 1.0.0, identify `herdr-delegator` version 3.7.0, and contain client-specific OMP data only under `extensions.io.github.edgar-min.herdr-delegator`.
...
- **PKG-003**: Agent Plugins portable authority MUST remain `plugin.json`, `skills/`, and `mcp.json`. `package.json` MUST remain npm/current-OMP compatibility metadata with version 3.7.0, direct runtime dependencies, and only the namespaced `omp.extensions` entry.
...
- **ACC-005 — Busy-lane queueing**: A second same-responsibility assignment queues while the lane is active and does not create another worker; an `urgent` add on that busy lane lands at the queue head, reports `data.queue_position` `0`, changes nothing about the running assignment, and leaves no trace of its urgency in any record.
...
- [ ] Agent Plugins `plugin.json`, Agent Skills frontmatter, package metadata, and skill metadata identify version 3.7.0.
...
- [ ] `urgent` is accepted only by `add`, inserts at the queue head, is echoed back solely as `data.queue_position` merged with that call's gate observations, and appears in no artifact, identifier, label, registry key, or schema version (RSP-003, ASN-015, ASN-016, ACC-005).
- [ ] The `wake_worker` subject names the queue head when the queue is non-empty, keeps the `queued`/`orch-response`/`completion` reason tokens unchanged, accepts an optional `assignment_id` only for an assignment the lane holds active or queued, and reports anything else as `target_unresolved` with one journal row and no bell (MSG-001, MSG-002).
...
- [ ] On the cadence park, one `emergency` add registers and writes `emergency-audit-1.md` while the run stays parked and a queued head stays queued; a non-emergency add is still refused with the carve-out named in its recovery; a second emergency add is refused by name until the verdict lands; a non-cadence park refuses the claim; the auditor's bell reaches the ORCH pane while the verdict is still read only from the document; and a server built before the feature reads the same registry unchanged.
...
- [ ] A run whose `handoff.md` is absent, or present without the `Inherited claims` section, dispatches exactly as before; a claims table with a vocabulary-violating, incomplete, or one-directional row is refused with the row named, a `measured` row observed at another commit is refused as stale, a non-Git project directory keeps the syntactic refusals while naming the skipped freshness check, and a passing gate leaves the registry unchanged; a successful `close` adds exactly one `metered:` ledger line.
