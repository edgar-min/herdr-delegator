...
This document is the normative architecture and Markdown review artifact for `herdr-delegator` 3.2.0 and the bundled `herdr-delegation` skill 3.2.0.
...
- **ID-002**: The package and plugin version MUST be `3.2.0`.
- **ID-003**: The public skill MUST be named `herdr-delegation` and versioned `3.2.0` under frontmatter metadata.
...
- **PKG-001**: Root `plugin.json` MUST conform to Agent Plugins 1.0.0, identify `herdr-delegator` version 3.2.0, and contain client-specific OMP data only under `extensions.io.github.edgar-min.herdr-delegator`.
...
- **PKG-003**: Agent Plugins portable authority MUST remain `plugin.json`, `skills/`, and `mcp.json`. `package.json` MUST remain npm/current-OMP compatibility metadata with version 3.2.0, direct runtime dependencies, and only the namespaced `omp.extensions` entry.
...
- [ ] Agent Plugins `plugin.json`, Agent Skills frontmatter, package metadata, and skill metadata identify version 3.2.0.
