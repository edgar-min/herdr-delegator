...
async function main(): Promise<void> {
  ...
  server.registerTool("herdr_track", { description: "Open a track — the single atomic op that creates its Herdr space and run, fixes the bounded mandate, spawns the ORCH pane pre-aligned to the configured orchestrator role, and records the ORCH birth that is the run's only command identity. The opening session is retired for that track at birth: direct the user to the named ORCH pane. Also inspects a track (including its budget), extends its budget against a bounded justification judged by a server-spawned clean auditor, safely closes it, and keeps the legacy init plus start_orchestrator pair for runs created before open.", inputSchema: herdrTrackInputShape }, async (input) => response(await tools.track(herdrTrackSchema.parse(input))));
