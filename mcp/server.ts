...
async function main(): Promise<void> {
  ...
  server.registerTool("herdr_track", { description: "Check a mandate draft and open a track. `check` runs the whole mandate check on an inline mandate — schema, pinned entry protocol, contract pin, entry.binds resolution and gate conjuncts, judged-unit style, substrate paths, plus the Jev routing/consistency/coverage/noise judgments when a key is present — and returns data.lines[] with data.verdict; it resolves no run and writes nothing, so a draft never has to be written into the user's project. `open` is the single atomic op that re-runs that check and refuses a FAILED verdict as mandate_check_failed, then creates the track's Herdr space and run, fixes the bounded mandate, spawns the ORCH pane pre-aligned to the configured orchestrator role, and records the ORCH birth that is the run's only command identity. The opening session is retired for that track at birth: direct the user to the named ORCH pane. Also inspects a track (including its budget), extends its budget on either axis — requested_tokens, requested_minutes, or both — against a bounded justification judged by a server-spawned clean auditor, revives a run by resuming its birth session or — with the user's written approval — reborn at generation+1, safely closes it, and keeps the legacy init plus start_orchestrator pair for runs created before open. A verdict is the decision, not its application: budget_extend and inspect both report per-axis granted, effective_cap, applied, usable, the park reason, one release_condition, and the exact clamp field and value a human must write.", inputSchema: herdrTrackInputShape }, guarded("herdr_track", herdrTrackSchema, (parsed) => tools.track(parsed)));
  server.registerTool("herdr_assignment", { description: "Route a goal before anything is written, preflight or add a canonical assignment on a persistent responsibility lane, or wait on its active assignment. `route` takes only the goal (and optional ownership, dependencies, boundaries) and returns the routing judge's verdict with a next step; call it before an assignment, a host subagent, or the delegation rules are read. The `action` field publishes the whole authoring contract: the artifact's frontmatter and section grammar with its bounds, the optional trailing `# References` section that pins documents by hash, the 0444 seal `add` applies, and the exact completion block that settles. Read it before writing an assignment file rather than after a preflight refuses one. There is no response action: answer a worker by appending an [ORCH Response] block to its lane report and ringing herdr_message wake_worker.", inputSchema: herdrAssignmentInputShape }, guarded("herdr_assignment", herdrAssignmentSchema, (parsed) => tools.assignment(parsed)));
