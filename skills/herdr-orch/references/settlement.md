...
in `supervision.md`: focused checks at each assignment boundary and one integration
verification at the integration boundary, executed by a delegate, never by you. Record
why the evidence satisfies each completion condition, any accepted warning, and any
recovery required. A tool-accepted completion proves report shape and identity, not
correctness.
