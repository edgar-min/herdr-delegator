...
export async function resolveBlock(params: ToolParams, signal?: AbortSignal): Promise<WorkerResult> {
  const timeoutMs = normalizeTimeout(params.timeout_ms);
  const coordinate = await resolveRunCoordinate(params.track_id, params.run_id);
  const runPath = coordinate.runPath;
  const workerId = canonicalWorkerId(params.worker_id);
  const workerKey = sha256(`${runPath}\0${workerId}`);
  const { registryPath } = registryPaths(runPath);
  const expectedSeq = params.expected_state_change_seq;
  if (typeof expectedSeq !== "number" || !Number.isSafeInteger(expectedSeq) || expectedSeq < 0) {
    throw new ContractError(
      "invalid_expected_state_change_seq",
      "expected_state_change_seq must be a non-negative safe integer.",
      "validate",
    );
  }
  const response = params.response;
  if (!isObject(response) || (response.kind !== "text" && response.kind !== "keys")) {
    throw new ContractError("invalid_response", "response must use the text or keys form.", "validate");
  }
  let responseArgs: { kind: "text"; text: string } | { kind: "keys"; keys: string[] };
  if (response.kind === "text") {
    if (
      typeof response.text !== "string" ||
      !response.text.length ||
      response.text.length > MAX_TEXT_RESPONSE ||
      response.text.includes("\0")
    ) {
      throw new ContractError(
        "invalid_response_text",
        `A text response must contain 1..${MAX_TEXT_RESPONSE} characters and no NUL.`,
        "validate",
      );
    }
    responseArgs = { kind: "text", text: response.text };
  } else {
    if (!Array.isArray(response.keys) || response.keys.length < 1 || response.keys.length > 20) {
      throw new ContractError("invalid_response_keys", "The keys response is outside the count limit.", "validate");
    }
    const keys: string[] = [];
    for (const key of response.keys) {
      if (typeof key !== "string" || !KEY_ALLOWLIST[key]) {
        throw new ContractError("invalid_response_keys", "The keys response contains a value outside the allowlist.", "validate");
      }
      keys.push(key);
    }
    responseArgs = { kind: "keys", keys };
  }
  const { binary } = await requireHerdrEnvironment();
  const focusBefore = await captureFocus(binary, signal);
  const owned: OwnedFocus = { tab_ids: new Set(), pane_ids: new Set() };
  ...
  try {
    const prepared = await withRegistryLock(runPath, timeoutMs, async (registry, targetRegistryPath) => {
      const run = await assertRunWorkspaceLive(binary, registry, timeoutMs, signal, runPath, coordinate.manifest.cwd);
      owned.workspace_id = run.workspace_id;
      if (run.anchor_tab_id) owned.tab_ids.add(run.anchor_tab_id);
      if (run.anchor_pane_id) owned.pane_ids.add(run.anchor_pane_id);
      const record = registry.workers[workerKey];
      if (!record?.root_pane_id || !record.agent_name) {
        throw new ContractError("worker_not_ensured", "The worker is not ready for block resolution.", "resolve_prepare");
      }
      assertRecordIdentity(record, runPath, workerId, workerKey);
      if (record.workspace_id !== run.workspace_id) {
        throw new ContractError("identity_conflict", "The worker is outside the owned run workspace.", "resolve_prepare");
      }
      if (record.tab_id) owned.tab_ids.add(record.tab_id);
      owned.pane_ids.add(record.root_pane_id);
      const live = await getLiveAgent(binary, record.agent_name, Math.min(timeoutMs, 15_000), signal);
      if (!live.ok) throw commandError(live, "resolve_prepare", "Call inspect_worker to refresh the state.");
      assertAgentBelongsToRecord(record, live.data);
      await updateRecordFromObservation(record, live.data);
      const liveSeq = firstNumber(live.data, ["state_change_seq"]);
      if (record.state !== "blocked" || liveSeq !== expectedSeq) {
        throw new ContractError(
          "blocked_state_changed",
          "The live agent is not blocked or its state_change_seq differs from the expected value.",
          "resolve_prepare",
          { recovery: "Read the latest state and sequence with inspect_worker before deciding on a response." },
        );
      }
      if (record.resolving_state_change_seq === expectedSeq) {
        throw new ContractError(
          "response_effect_ambiguous",
          "A response for the same blocked sequence has already started.",
          "resolve_prepare",
          {
            retryable: true,
            ambiguousEffect: true,
            recovery: "Do not resend the response; inspect its effect with inspect_worker and agent wait.",
          },
        );
      }
      record.resolving_state_change_seq = Number(expectedSeq);
      record.updated_at = nowIso();
      await writeRegistryAtomic(targetRegistryPath, registry);
      return record;
    });
    ...
    if (responseArgs.kind === "keys") {
      sent = await runHerdr(
        binary,
        ["agent", "send-keys", prepared.agent_name, ...responseArgs.keys],
        timeoutMs,
        signal,
      );
    } else {
      sent = await runHerdr(
        binary,
        ["pane", "send-text", prepared.root_pane_id!, responseArgs.text],
        timeoutMs,
        signal,
      );
      if (sent.ok) {
        sent = await runHerdr(
          binary,
          ["agent", "send-keys", prepared.agent_name, "enter"],
          timeoutMs,
          signal,
        );
      }
    ...
    if (!sent.ok) {
      throw commandError(
        sent,
        "resolve_send",
        "Do not resend a response for the same sequence; inspect its effect with inspect_worker and pane read.",
        true,
      );
    }
    ...
    if (!settled.ok) {
      throw commandError(
        settled,
        "resolve_wait",
        "The response was sent; do not resend it, and confirm settlement with inspect_worker and agent wait.",
        true,
      );
    }

    const record = await withRegistryLock(runPath, timeoutMs, async (registry, targetRegistryPath) => {
      await assertRunWorkspaceLive(binary, registry, timeoutMs, signal, runPath, coordinate.manifest.cwd);
      const current = registry.workers[workerKey];
      if (!current) throw new ContractError("worker_missing", "The registry record is missing after block resolution.", "registry");
      await updateRecordFromObservation(current, settled.data);
      delete current.resolving_state_change_seq;
      await writeRegistryAtomic(targetRegistryPath, registry);
      return current;
    });
    const afterSeq = firstNumber(settled.data, ["state_change_seq"]);
    result = {
      ok: true,
      operation: "resolve_block",
      worker_key: workerKey,
      state: publicState(record.state),
      retryable: false,
      registry_path: registryPath,
      worker: publicWorker(record),
      observation: {
        response_state_change_seq_before: expectedSeq,
        response_state_change_seq_after: afterSeq,
        working_observed: workingWait.ok,
        settle_state: record.state,
      },
    };
  } finally {
    focusRestoration = await restoreFocus(binary, focusBefore, owned);
